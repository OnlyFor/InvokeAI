...
import type { CollectInvocation, Edge, Invocation, IPAdapterInvocation, NonNullableGraph } from 'services/api/types';
...
export const addRegionalPromptsToGraph = async (state: RootState, graph: NonNullableGraph, denoiseNodeId: string) => {
  ...
  const layers = state.regionalPrompts.present.layers
    ...
    .filter((l) => {
      ...
      const hasIPAdapter = l.ipAdapterId !== null;
  ...
  for (const layer of layers) {
    ...
    if (layer.positivePrompt) {
      ...
      const regionalPositiveCondNode: Invocation<'sdxl_compel_prompt'> | Invocation<'compel'> = isSDXL
        ? {
    ...
    if (layer.negativePrompt) {
      ...
      const regionalNegativeCondNode: Invocation<'sdxl_compel_prompt'> | Invocation<'compel'> = isSDXL
        ? {
            type: 'sdxl_compel_prompt',
    ...
    if (layer.autoNegative === 'invert' && layer.positivePrompt) {
      ...
      // positive prompt
      const regionalPositiveCondInvertedNode: Invocation<'sdxl_compel_prompt'> | Invocation<'compel'> = isSDXL
        ? {
            type: 'sdxl_compel_prompt',
    ...
    if (layer.ipAdapterId !== null) {
      const ipAdapter = selectAllIPAdapters(state.controlAdapters).find((ca) => ca.id === layer.ipAdapterId);
